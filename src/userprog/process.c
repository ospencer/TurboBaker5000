...
process_exit (void)
{
  ...
  int i;
  for(i = 2; i < sizeof(cur->fds)/sizeof(cur->fds[0]); i++)
  {
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      uint8_t *kpage = palloc_get_page (PAL_USER);//add_frame_entry(PAL_USER);// palloc_get_page (PAL_USER);
      if (kpage == NULL)
        return false;
...
setup_stack (void **esp, const char *input) 
{
  ...
  if (input_cpy == NULL)
    return TID_ERROR;
  strlcpy(input_cpy, input, PGSIZE);
  kpage = palloc_get_page (PAL_USER | PAL_ZERO); ///add_frame_entry(PAL_ZERO);// palloc_get_page (PAL_USER | PAL_ZERO);
